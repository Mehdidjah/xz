...
static int stest_json_output = 0;
static int stest_progress_enabled = 0;
static int stest_total_asserts = 0;
static double stest_test_time = 0.0;
static double stest_total_time = 0.0;
static clock_t stest_test_start_time;
static clock_t stest_total_start_time;
...
void stest_assert_null(const void *ptr, const char *function, unsigned int line) {
  char s[STEST_PRINT_BUFFER_SIZE];
  sprintf(s, "Expected NULL but was %p", ptr);
  stest_simple_test_result(ptr == NULL, s, function, line);
}

void stest_assert_not_null(const void *ptr, const char *function, unsigned int line) {
  char s[STEST_PRINT_BUFFER_SIZE];
  sprintf(s, "Expected non-NULL but was NULL");
  stest_simple_test_result(ptr != NULL, s, function, line);
}

void stest_assert_ptr_equal(const void *expected, const void *actual, const char *function, unsigned int line) {
  char s[STEST_PRINT_BUFFER_SIZE];
  sprintf(s, "Expected pointer %p but was %p", expected, actual);
  stest_simple_test_result(expected == actual, s, function, line);
}

void stest_assert_ptr_not_equal(const void *expected, const void *actual, const char *function, unsigned int line) {
  char s[STEST_PRINT_BUFFER_SIZE];
  sprintf(s, "Expected pointer not to be %p but was %p", expected, actual);
  stest_simple_test_result(expected != actual, s, function, line);
}
...
void stest_test(const char *test, void (*test_function)(void)) {
  ...
  stest_test_start_time = clock();
  skip_failed_test = setjmp(env);
  if(!skip_failed_test)
    test_function();
  
  stest_test_time = ((double)(clock() - stest_test_start_time)) / CLOCKS_PER_SEC;

  stest_teardown();
  stest_suite_teardown();
  
  if(stest_progress_enabled && !stest_machine_readable && !stest_json_output) {
    printf("Running: %s (%.3fs) [Tests: %d, Passed: %d, Failed: %d]\r", 
           test, stest_test_time, stests_run + 1, stests_passed, stests_failed);
    fflush(stdout);
  }
...
int run_tests(stest_void_void tests) {
  char s[40];
  stest_total_start_time = clock();
  stest_total_asserts = 0;
  
  tests();
  
  stest_total_time = ((double)(clock() - stest_total_start_time)) / CLOCKS_PER_SEC;
  
  if(stest_json_output) {
    printf("{\"summary\":{\"total\":%d,\"passed\":%d,\"failed\":%d,\"asserts\":%d,\"time\":%.3f}}\n",
           stests_run, stests_passed, stests_failed, stest_total_asserts, stest_total_time);
    return STEST_RET_FAILED_COUNT(stests_failed);
  }
  
  if(stest_progress_enabled && !stest_machine_readable) {
    printf("\n");
  }
  ...
  sprintf(s, "Total time: %.3fs | Assertions: %d", stest_total_time, stest_total_asserts);
  stest_header_printer(s, strlen(s), stest_screen_width, ' ');
  printf("\r\n");
...
void stest_show_help(void) {
  ...
  printf("\t-j:\twill output test results in JSON format\r\n");
  printf("\t-p:\twill show progress indicator during test run\r\n");
}
...
void stest_interpret_commandline(stest_testrunner_t *runner) {
  ...
  for(arg = 1; (arg < runner->argc) && (runner->action != STEST_DO_ABORT);
    ...
    else if(!strncmp(runner->argv[arg], "-j", sizeof("-j")))
      stest_json_output = 1;
    else if(!strncmp(runner->argv[arg], "-p", sizeof("-p")))
      stest_progress_enabled = 1;
